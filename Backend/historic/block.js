...
class Block {
    ...
    this.data = data; 
    this.previousHash = previousHash; //is it really used ?
...
function exportChainOnlyToExcel(chain, out_file){
    chain_info = [
        {
            name: "sheet",
            data: [["index", "timestamp", "data", "hash"]]
    ...
    for(let i=1;i<=chain.length-1;i++){
        chain_info[0].data.push([chain[i].index, chain[i].timestamp, chain[i].data, chain[i].hash])
    }
    buffer = XLSX_test.build(chain_info)
    fs.writeFile(out_file, buffer, function(err){
        if(err)
