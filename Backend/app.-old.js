 ...
 */
		...
		try {
			...
			//retrieve all infos from excel file
			app.get("/initiate", async (req, res) => {
				console.log('*****read ledger file*****')
				let fileContents = fs.readFileSync(ledgerFile, 'utf8'); // read ledger file
				let ledger = yaml.load(fileContents);
			...
			//save the updates in excel file hist_DR
			app.get("/saveStatus", async (req, res) => {
				...
				for(let i=0;i<retry_cnt;i++){
					try {
						...
						//TO DO: retrieve updates not by using HLF API
						...
						let yamlStr = yaml.dump(ledger); //used to convert from js to YAML format
			...
			//save in excel file specific to the update => no nedd in the new architecture
			...
			//export in word file => specific to each update. No need in the new architecture
			/* app.get("/exportBlockchain", async (req, res) => {
				console.log("app exportBlockchain");
				let timestamp = getTimestamp(); // YYYYMMDD_hhmm
				let outFileName = `./historic/hist_${ontologyInfo['Name'].split('.')[0]}_${timestamp}.docx`;
				let result;
				for(let i=0;i<retry_cnt;i++){
					try {
						let chain = JSON.parse(await contract.evaluateTransaction("GetBlockchain"))
